...
class User(object):
    ...
    def is_valid_user(user):
        return Utility.is_solid_dict(user) and \
               ...
               'uuid_md5' in user and len(user['uuid_md5']) == 32 and \
               'ifna_md5' in user and len(user['ifna_md5']) == 32 and \
               'duid_md5' in user and len(user['duid_md5']) == 32 and \
...
class Tracking(object):
    _commands = {'log_refresh': "echo 'user_id:{0}|ip:{1}|date:{2}' >> /etc/feiti/console/track/refresh",
                 'log_signup': "echo 'date:{0}|ip:{1}|uuid:{2}|ifna:{3}|duid:{4}' >> /etc/feiti/console/track/signup",
                 'log_update_password': "echo 'user_id:{0}|ip:{1}|password:{2}|date:{3}' >> /etc/feiti/console/track/update_password"
                 }
    ...
    @staticmethod
    def log_signup(ip, uuid, ifna, duid):
        now = Utility.date_to_str(Utility.get_utc_date(), Utility.DATE_FORMAT_FOR_ID_USAGE)
        return LinuxCommand.execute_query(Tracking._commands['log_signup'].format(now, ip, uuid, ifna, duid))

    @staticmethod
    def log_update_password(user, ip):
        if User.is_valid_user(user):
            now = Utility.date_to_str(Utility.get_utc_date(), Utility.DATE_FORMAT_FOR_ID_USAGE)
            return LinuxCommand.execute_query(Tracking._commands['log_update_password'].format(user['user_id'], ip, user['password'], now))
        return False
...
class FeiTiAPIHandler(BaseHTTPRequestHandler):
    ...
    def _token_handler(self, token, check_expire=True):
        ...
        if user_id:
            if user_id == -1:
                ...
                # self.send_error(FeiTiAPIHandler.ERROR_CODE_COMMON_TOKEN_TIMEOUT)
            else:
                return user_id
        else:
            self._error_handler(FeiTiAPIHandler.ERROR_CODE_COMMON_INVALID_TOKEN)
            # self.send_error(FeiTiAPIHandler.ERROR_CODE_COMMON_INVALID_TOKEN)

    def _sign_up_handler(self, ip, uuid, ifna, duid):
        Tracking.log_signup(ip, uuid, ifna, duid)
        user = User.get_user_by_ifna(ifna) if ifna != '00000000-0000-0000-0000-000000000000' else None
        password = User.new_password()
        if not user:
            ...
            if not user:
                ...
                if not user:
                    user = User.new_guest_user(ip, uuid, ifna, duid)
    ...
    def _user_update_password(self, user_id, password):
        ...
        if user:
            ...
            Tracking.log_update_password(user, self.client_ip)
            token = Token.user_id_to_token(user_id)
        ...
        if command and client_ip:
            if command == FeiTiAPIHandler.COMMAND_IOS_SIGN_UP:
                if self._verify_request_params(params, ['uuid', 'duid', 'ifna']):
                    self._sign_up_handler(client_ip, params['uuid'], params['ifna'], params['duid'])
